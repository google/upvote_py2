...
class GetSantaHostKeysForUserTest(basetest.UpvoteTestCase):
  ...
  def testSuccess(self):
    ...
    santa_host_key_1 = test_utils.CreateSantaHost(
        primary_user=user_1.nickname).key

    # User #2 has two hosts.
    santa_host_key_2 = test_utils.CreateSantaHost(
        primary_user=user_2.nickname).key
    test_utils.CreateSantaHost(primary_user=user_2.nickname)

    # User #1 has run something on a host of User #2.
    blockable = test_utils.CreateSantaBlockable()
    test_utils.CreateSantaEvent(
        blockable, executing_user=user_1.nickname,
        host_id=santa_host_key_2.id())

    # Should get back two hosts for User #1.
    expected_host_keys = sorted([santa_host_key_1, santa_host_key_2])
    actual_host_keys = sorted(model_utils.GetSantaHostKeysForUser(user_1))
...
class GetSantaHostIdsForUserTest(basetest.UpvoteTestCase):
  ...
  def testSuccess(self):
    ...
    actual_host_ids = sorted(model_utils.GetSantaHostIdsForUser(user_1))
...
class IsSantaHostAssociatedWithUserTest(basetest.UpvoteTestCase):
  ...
  def testAssociated_HasEvent(self):
    ...
    # Create a host not owned by User #1.
    host = test_utils.CreateSantaHost(primary_user=user_2.nickname)

    # Create an Event which was generated by User #1.
    blockable = test_utils.CreateSantaBlockable()
    test_utils.CreateSantaEvent(
        blockable, executing_user=user_1.nickname, host_id=host.key.id())

    self.assertTrue(model_utils.IsSantaHostAssociatedWithUser(host, user_1))
